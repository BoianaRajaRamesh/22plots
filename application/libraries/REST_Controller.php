...
abstract class REST_Controller extends \CI_Controller
{
    ...
    function throw_error($msg, $status = 400)
    {
        $data = array(
            'status' => $status,
            "message" => $msg
    ...
    function generateToken($user_id, $phone_number)
    {
        $data = array(
            'user_id' => $user_id,
        ...
        $token = base64_encode($jsonString . $key);
    ...
    function decodeToken($token)
    {
        ...
        try {
            $decodedToken = $this->jwt->decode($token, $key);
            return $decodedToken;
        } catch (Exception $e) {
            // Token is invalid
            return false;
    ...
    function checkAuth($userID = '')
    {
        if ($this->input->get_request_header('Authorization')) {
            ...
            $this->throw_error('Authorization header is missing', 401);
        }
    }
    ...
    private function validateToken($token)
    {
        ...
        // Extract the key from the end of the decoded token
        $extractedKey = substr($decodedToken, -strlen($key));

        // Check if the extracted key matches the expected key
        if ($extractedKey !== $key) {
            $this->throw_error('Invalid token', 401);
        }

        // Remove the key from the end of the decoded token
        $jsonString = substr($decodedToken, 0, -strlen($key));
