    ...
    public function saveIndependentHouse_post()
    {
        ...
        foreach ($apartmentData as $field => $value) {
            ...
            if ($field == 'govt_approval' || $field == 'contact_for_price' || $field == 'negotiable' || $field == 'brokerage' || $field == 'loan_availability' || $field == 'need_help') {
                ...
                if ($value == true) {
                    $apartmentData[$field] = true;
                } else {
                    $apartmentData[$field] = false;
                }
            }
        }
        ...
        $apartmentData['banner_img'] = $banner_img;
        if (empty($this->post('bhk_type'))) {
            $this->throw_error('bhk_type Required');
        }
        if (empty($this->post('bathrooms'))) {
            $this->throw_error('bathrooms Required');
        }
        if (empty($this->post('balcony'))) {
            $this->throw_error('balcony Required');
        }
        $image_paths = [];
        ...
        if ($last_id) {
            ...
            if (isset($_FILES['floor_plan']['name'])) {
                $floor_plan = $this->file_upload($_FILES['floor_plan'], 'property_plan');
            } else {
                $floor_plan = "";
            }
            $flats_config = array(
                "property_id" => $last_id,
                "bhk_type" => $this->post('bhk_type'),
                "floor_num" => $this->post('floor_num'),
                "facing" => $this->post('facing'),
                "bathrooms" => $this->post('bathrooms'),
                "balcony" => $this->post('balcony'),
                "super_builtup_area" => $this->post('super_builtup_area'),
                "builtup_area" => $this->post('builtup_area'),
                "carpet_area" => $this->post('carpet_area'),
                "floor_plan" => $floor_plan,
                "price" => $this->post('price')
            );
            $this->common_api_model->add_data('flats_config', $flats_config);
